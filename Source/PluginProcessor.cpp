...
#endif
{
    ...
    for (int i = 0; i < 5; i++) { synth.addVoice(new SynthVoice()); }
}

SAP::~SynthAudioProcessor() {}

//==============================================================================
const String SAP::getName() const { return JucePlugin_Name; }
...
void SAP::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    ...
    for (int i = 0; i < synth.getNumVoices(); i++) {
        if (auto voice = dynamic_cast<SynthVoice*>(synth.getVoice(i))) { voice->prepareToPlay(sampleRate, samplesPerBlock, getTotalNumOutputChannels()); }
    }
}
...
AudioProcessorEditor* SAP::createEditor() { return new SynthAudioProcessorEditor(*this); }
...
void SAP::setFilterParams()
{
    ...
    for (int i = 0; i < synth.getNumVoices(); ++i) {
        if (auto voice = dynamic_cast<SynthVoice*>(synth.getVoice(i))) { voice->updateModParams(filterType, filterCutoff, filterResonance, adsrDepth, lfoFreq, lfoDepth); }
    }
}
