...
bool SynthVoice::canPlaySound(SynthesiserSound* sound) { return dynamic_cast<SynthesiserSound*>(sound) != nullptr; }
...
void SynthVoice::renderNextBlock(AudioBuffer<float>& outputBuffer, int startSample, int numSamples)
{
    ...
    for (int channel = 0; channel < synthBuffer.getNumChannels(); ++channel) {
        ...
        for (int s = 0; s < synthBuffer.getNumSamples(); ++s) { buffer[s] = osc1[channel].processNextSample(buffer[s]) + osc2[channel].processNextSample(buffer[s]); }
    }
    ...
    for (int channel = 0; channel < synthBuffer.getNumChannels(); ++channel) {
        ...
        for (int s = 0; s < synthBuffer.getNumSamples(); ++s) { buffer[s] = filter[channel].processNextSample(channel, synthBuffer.getSample(channel, s)); }
    }
...
void SynthVoice::updateModParams(const int filterType, const float filterCutoff, const float filterResonance, const float adsrDepth, const float lfoFreq, const float lfoDepth)
{
    ...
    for (int channel = 0; channel < numChannelsToProcess; ++channel) { filter[channel].setParams(filterType, cutoff, filterResonance); }
}
